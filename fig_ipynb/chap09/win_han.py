...
def win_hanning(n, M=16, WB=0, fillzero=False):
  '''Rectangular window.
...
def extract_by_win_hanning(n, f, M=16, WB=0, fillzero=False):
...
def win_hanning_spectral(M=16, omg_shift=0, l_range=-2*np.pi, r_range=2*np.pi, step=0.001):
  '''Rectangular window.
       M: window size; dafault = 16
  ...
  W_l = (np.sin((M/2)*(omg-2*np.pi/M))/np.sin((1/2)*(omg-2*np.pi/M)))*np.exp((omg-2*np.pi/M)*(-(M-1)/2)*1j)
  W_r = (np.sin((M/2)*(omg+2*np.pi/M))/np.sin((1/2)*(omg+2*np.pi/M)))*np.exp((omg+2*np.pi/M)*(-(M-1)/2)*1j)
  W = 0.5*W - 0.25*W_l - 0.25*W_r
